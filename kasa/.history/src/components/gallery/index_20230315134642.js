...
function Gallery() { 
  ...
  // Initialisation de l'index de l'image courante à afficher

  const [current, setCurrent] = useState(0); 
  // Récupération de l'index du logement dans la liste
  ...
  const prevSlide = () => { 
    ...
    // Récupération de l'ID du logement précédent
    const prevId = locatelist[prevIndex].id; 
    // Mise à jour de l'index de l'image courante à afficher
    setCurrent(current === 0 ? pictures.length - 1 : current - 1); 
    // Redirection vers la page du logement précédent
