...
import { useParams } from "react-router-dom"; // Importation de useParams depuis la bibliothèque 'react-router-dom'
import "./gallery.css"; // Importation de la feuille de style CSS pour cette galerie
import arrowback from "../../asssets/arrow/arrow_back.png"; // Importation de l'image de la flèche gauche
import arrownext from "../../asssets/arrow/arrow_next.png"; // Importation de l'image de la flèche droite
import locatelist from "../../datas/locatelist.json"; // Importation de la liste des logements depuis un fichier JSON

function Gallery() { // Définition du composant Gallery
  const { id } = useParams(); // Récupération de l'ID du logement depuis les paramètres de l'URL
  const locatePict = locatelist.find((object) => object.id === id); // Récupération des images correspondantes au logement
  const { pictures } = locatePict; // Stockage des images dans une variable
