...
function Gallery() {
  ...
  nst prevSlide = () => {
    ...
    if (prevId === undefined) {
      ...
      window.location.replace(`/product/${locatelist[0].id}`);
    } else {
      window.location.replace(`/product/${prevId}`);
    }
  };
  ...
  const nextSlide = () => {
    ...
    if (nextId === undefined) {
      ...
      window.location.replace(`/product/${locatelist[locatelist.length - 1].id}`);
    } else {
